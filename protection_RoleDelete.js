...
module.exports = {
  ...
  execute(client) {
    client.on(Events.RoleDelete, async (role) => {
      const audit = await role.guild.fetchAuditLogs({ action: 32, limit: 1 });
      const info = audit.entries.first();
      
      if (info.action === 32 && info.target?.id === role.id && info.executor) {
        if (info.executor?.id === client.user?.id || info.executor?.id === role.guild.ownerId) return;

        let limit = protectiondb.get("actionLimit_" + "delete_roles_" + role.guild?.id) || 5;
        let automode = protectiondb.get("autoMode_" + "delete_roles_" + role.guild?.id) || "delete_roles";
        ...
        if (whitelistCheck && (whitelistCheck.includes("delete_role") || whitelistCheck.includes("all"))) {
          if (logID) {
          ...
          let userData = protectiondb.get("protectionData_" + "delete_roles_" + role.guild?.id + "_" + info.executor?.id) || 0;
          ...
          protectiondb.set("protectionData_" + "delete_roles_" + role.guild?.id + "_" + info.executor?.id, userData + 1).then(() => {
